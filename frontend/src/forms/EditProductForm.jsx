...
export function EditProductForm({handleUpdateProduct, product, form, handleChange}) {

    const [toggleEdit, setToggleEdit] = useState(false);
    

    return (<>
            <div className="block hover:cursor-pointer" onClick={()=> setToggleEdit(!toggleEdit)}>Edit
            </div>
            {toggleEdit ? 
                (
                    <>
                                        ...
                                        <form onSubmit={()=> handleUpdateProduct(product.id)} className="flex flex-col">
                                            ...
                                            <div className="flex items-center justify-end p-6 border-t border-solid border-blueGray-200 rounded-b">
                                                <button
                                                    ...
                                                    type="button" onClick={() => setToggleEdit(false)}>
                                                    Close
                                                </button>
                                                <button
